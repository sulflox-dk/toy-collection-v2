                    ...
                    <div class="card h-100 universe-select-card shadow-sm rounded-4 overflow-hidden" onclick="CatalogWizard.goToStep2(<?= $u['id'] ?>)">
                        <div class="ratio ratio-1x1 bg-white p-3">
                            <?php if (!empty($u['image_path'])): ?>
                                <img src="<?= $baseUrl . htmlspecialchars($u['image_path']) ?>" alt="<?= htmlspecialchars($u['name']) ?>"
                                     class="object-fit-contain"
                                     onerror="this.style.display='none'; this.nextElementSibling.classList.remove('d-none');">
                            <?php endif; ?>

                            <div class="<?= empty($u['image_path']) ? '' : 'd-none' ?> w-100 h-100 d-flex align-items-center justify-content-center bg-light text-center">
